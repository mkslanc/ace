    ...
    compute(originalLines, modifiedLines, opts) {
        ...
        /*chunks.forEach(function(diff) {
            ...
            if (diff.origStart == diff.origEnd) {
                ...
                let inlineDiff = dmp.diff_main(
                    originalLines.slice(diff.origStart, diff.origEnd).join("\n"),
                ...
                let modLine  = diff.editStart, modCol  = 0;

                inlineDiff.forEach(([changeType, text]) => {
                    if (!text) return;                  // nothing to do for empty chunks
                    ...
                    if (changeType === DIFF_DELETE || changeType === DIFF_INSERT) {
                        inlineChanges.push({
                            ...
                            originalEndLineNumber:     changeType === DIFF_DELETE ? origLine + rowCount : origLine,
                            originalEndColumn:         changeType === DIFF_DELETE ? colCount        : origCol,

                           /!* modifiedStartLineNumber:   modLine,
                            modifiedStartColumn:       modCol,
                            modifiedEndLineNumber:     changeType === DIFF_INSERT ? modLine + rowCount : modLine,
                            modifiedEndColumn:         changeType === DIFF_INSERT ? colCount        : modCol*!/
                        });
                    }
                    ...
                    if (changeType !== DIFF_DELETE) {
                        modLine += rowCount;
                        modCol  = rowCount ? colCount : (modCol + colCount);
                    }
                });
