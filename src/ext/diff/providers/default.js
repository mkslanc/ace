// autogenerated from vscode-diff-bundle with command build
'use strict';
...
class DefaultLinesDiffComputerWithSeparateRefinement extends DefaultLinesDiffComputer {
    constructor() {
        super(...arguments);
        this.lineDynamicProgrammingDiffing = new DynamicProgrammingDiffing();
        this.lineMyersDiffingAlgorithm = new MyersDiffAlgorithm();
    }
    computeDiff(originalLines, modifiedLines, options) {
        if (originalLines.length <= 1 && equals(originalLines, modifiedLines, (a, b) => a === b)) {
            return new LinesDiff([], [], false);
...
function toDetailedLineRangeMapping(sequenceDiff) {
...
function computeDiff(originalLines, modifiedLines, options) {
    ...
    return result.changes.map((change) => {
        return {
            ...
            inlinePending: !result.hitTimeout && change.original.length > 0 && change.modified.length > 0
        };
    });
}
...
class DiffProvider {
    ...
    refine(originalLines, modifiedLines, chunk, opts) {
        ...
        const computer = new DefaultLinesDiffComputerWithSeparateRefinement();
