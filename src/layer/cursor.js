...
class Cursor {
    ...
    getPixelPosition(position, onScreen) {
        ...
        var textWidth = this.config.textWidth(position.row, position.column);
        var cursorLeft = this.$padding + textWidth;

        var cursorTop = (pos.row - (onScreen ? this.config.firstRowScreen : 0)) * this.config.lineHeight;

        var cursorWidth = this.config.characterWidth;
        var line = this.session.getLine(position.row);
        if (position.column < line.length) {
            var nextWidth = this.config.textWidth(position.row, position.column + 1);
            cursorWidth = nextWidth - textWidth;
        }
